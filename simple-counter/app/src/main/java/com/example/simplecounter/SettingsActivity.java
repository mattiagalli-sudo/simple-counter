...
public class SettingsActivity extends Activity {
    ...
    private static final String[] CLOCK_POSITION_LABELS = {
            "Top left",
            "Top center",
            "Top right",
            "Bottom left",
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button resetOperationsButton = (Button) findViewById(R.id.resetOperationsButton);
        ...
        Spinner clockPositionSpinner = (Spinner) findViewById(R.id.clockPositionSpinner);
        ...
        resetOperationsButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        setupClockPositionSpinner(clockPositionSpinner);
    ...
    private void addOperation(EditText operationLabelInput, Spinner operationTypeSpinner, EditText multiplierInput) {
        try {
            ...
            if (Double.isNaN(operand) || Double.isInfinite(operand)) {
                toast("Enter a finite operation number");
                return;
            }
    ...
    private void setupClockPositionSpinner(Spinner spinner) {
        spinner.setAdapter(simpleAdapter(CLOCK_POSITION_LABELS));
        spinner.setSelection(clamp(prefs.getInt(KEY_CLOCK_POSITION, 0), 0, CLOCK_POSITION_LABELS.length - 1));
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                prefs.edit().putInt(KEY_CLOCK_POSITION, position).apply();
            }
    ...
    private void setupFontSpinner(Spinner spinner) {
        ...
        spinner.setSelection(clamp(prefs.getInt(KEY_FONT_STYLE, 0), 0, FONT_LABELS.length - 1));
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
    ...
    private void setupFontSizeSeekBar(SeekBar seekBar) {
        int size = clamp(prefs.getInt(KEY_COUNTER_FONT_SIZE, 72), 32, 160);
    ...
    private void setupClockSizeSeekBar(SeekBar seekBar) {
        int size = clamp(prefs.getInt(KEY_CLOCK_FONT_SIZE, 16), 10, 42);
    ...
    private void updateOperationsList() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (parts.length == 3) {
                int operationIndex = indexOf(OPERATION_VALUES, parts[1]);
                builder.append(i + 1)
                        .append(". ")
                        .append(parts[0].length() == 0 ? OPERATION_LABELS[operationIndex] : parts[0])
                        .append(" - ")
                        .append(OPERATION_LABELS[operationIndex])
                        .append(" ")
                        .append(parts[2])
                        .append("\n");
            }
